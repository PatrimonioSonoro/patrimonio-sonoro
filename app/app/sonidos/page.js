...
async function fetchPublicContents({ q } = {}) {
  ...
  let query = sb
    .from('contenidos')
    .select('id,title,description,region,created_at,image_public_url')
    .eq('status', 'published')
...
export default async function SonidosPage({ searchParams }) {
  ...
  return (
    <main>
      <PrivatePageHeader
        title="Estrategias"
        subtitle="Explora imágenes y materiales asociados a las diferentes estrategias."
      />

      <section className="py-8 bg-white">
        <div className="container mx-auto px-4">
          <div className="flex items-start justify-between gap-6 flex-col md:flex-row">
            <form action="/app/sonidos" className="w-full md:w-auto">
