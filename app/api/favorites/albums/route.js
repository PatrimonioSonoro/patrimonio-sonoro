...
export async function GET(req) {
  try {
    ...
    return NextResponse.json({ album_ids: (data || []).map((r) => r.album_id) }, { status: 200 });
  } catch (e) {
...
export async function POST(req) {
  try {
    ...
    const album_id = body?.album_id ? String(body.album_id) : null;
    ...
    if (favorite === null) {
      ...
      if (existing) {
        ...
        return NextResponse.json({ album_id, favorited: false }, { status: 200 });
      }

      const { error: insErr } = await authed.userClient.from("album_favorites").insert({ user_id: uid, album_id });
      if (insErr) return NextResponse.json({ error: "Database error" }, { status: 500 });
      return NextResponse.json({ album_id, favorited: true }, { status: 200 });
    }

    if (favorite) {
      const { error: upsertErr } = await authed.userClient
        .from("album_favorites")
        .upsert({ user_id: uid, album_id }, { onConflict: "user_id,album_id" });
